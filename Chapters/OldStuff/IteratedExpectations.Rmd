...
In some situations, we only observe a single outcome but can conceptualize an outcome as resulting from a two (or more) stage
...
### Law of Iterated Expectations {#S:AppB:LIE}
...
Given $p(y|x)$ the joint `r Gloss('pmf')` of $X$ and $Y$, the conditional expectation of $h(X,Y)$ given the event $X=x$ is defined as
...
As described in Section \@ref(S:IntroBayes), for conjugate distributions the posterior and the prior come from the same family of distributions. In insurance applications, this broadly occurs in a "family of distribution families" known as the linear exponential family which we introduce first.
...
### Conjugate Distributions {#S:IterExp:Conjugate}
